const YOUTUBE_API_KEY = "AIzaSyC9Ce8cKbeDz8ZXdx9ZJTIT0EwMrFptl5I"; // Remplace ta clé API YouTube
...
// Extraire l'ID vidéo depuis le lien (court + long + shorts)
function extractVideoID(url) {
  const regExp = /^.*(?:youtu.be\/|v\/|u\/\w\/|embed\/|shorts\/|watch\?v=)([^#&?]{11}).*/;
...
// Récupérer transcription via backend Vercel avec youtube-transcript
async function fetchTranscript(videoId) {
  try {
    const res = await fetch(`/api/transcript?videoId=${videoId}`);
    if (!res.ok) throw new Error("Transcription non disponible.");
    const data = await res.json();
    return data.transcript || "Transcription non disponible.";
  } catch (e) {
